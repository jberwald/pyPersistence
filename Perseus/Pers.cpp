...
bool buildSToplexFromFile (ifstream&, MComplex<CC,BC>&, SToplex<CC,double,BC>&);
bool buildNMFSToplexFromFile (ifstream&, MComplex<CC,BC>&, SToplex<CC,double,BC>& , bool);
...
int main(int argc, char* argv[])
{
    ...
    if (input_type == "debug")
    {
        ...
        mycub.addTopCube(coord,1); coord.clear();

        // 7
        coord.push_back(1); coord.push_back(2);
        mycub.addTopCube(coord,5); coord.clear();

        // 1
        coord.push_back(0); coord.push_back(1);
        mycub.addTopCube(coord,2); coord.clear();

        // 2
        coord.push_back(0); coord.push_back(2);
        mycub.addTopCube(coord,3); coord.clear();

        // 3
        coord.push_back(2); coord.push_back(0);
        mycub.addTopCube(coord,6); coord.clear();

        // 5
        coord.push_back(2); coord.push_back(1);
        mycub.addTopCube(coord,7); coord.clear();

        // ?
        coord.push_back(1); coord.push_back(0);
        mycub.addTopCube(coord,4); coord.clear();


        coord.push_back(2); coord.push_back(2);
        mycub.addTopCube(coord,1); coord.clear();

        cout<<mycub;

        mycub.ComputePersistence(outfile);
        return 0;
	...
	else if (input_type == "simtop")
	{
		...
		if (! buildSToplexFromFile (inf, *ccomp, stop) )
		{
			cout<<"\nFile "<<infile<<" exists but contains bogus data";
			return -3;
		}
	...
	{
		SToplex<CC, double, BC> stop;
		if (! buildNMFSToplexFromFile (inf, *ccomp, stop, false) )
		{
				cout<<"\nFile "<<infile<<" exists but contains bogus data";
				return -3;
		}
	...
	{
		SToplex<CC, double, BC> stop;
		if (! buildNMFSToplexFromFile (inf, *ccomp, stop, true) )
		{
				cout<<"\nFile "<<infile<<" exists but contains bogus data";
				return -3;
		}
    ...
    else if (input_type == "bary")
    {
        ...
        //stop.barySub(bcstop);
        // output file?
        outfile = infile.replace(infile.find("."),4,"_runstat.txt");
        ofstream outf(outfile.c_str());
        //ostream& outf = cout;

        // outf<<"\n\n******* Unreduced size of original: "<< ccomp->size(); //cin.get();
        ...
        for (num count = 0; count < totcount; count++)
        {
            ...
            if (eng == "r")
            {
                ReduceAndUpdate(mycomp,savegens,0.0);
            }
            else if (eng == "c")
            {
                CoreduceAndUpdate(mycomp,savegens,0.0);
            }
            else
            {
                AlternateAndUpdate_makeStat(mycomp,svec);
                //AlternateAndUpdate(mycomp,savegens,0.0,false,true);
            }
	...
	if(FLOWTALK) cout<<"Complex stored with "<<ccomp->size()<<" cells!";
	if (MAKEBPS) cin.get();

    // choose engine
    if (eng == "r")
    {
       ReduceAndUpdate(ccomp,savegens,0.0);
    }
    else if (eng == "c")
    {
       CoreduceAndUpdate(ccomp,savegens,0.0);
    }
    ...
    {
       AlternateAndUpdate(ccomp,savegens,0.0);
    }
...
// :
// :
bool buildBRIPSFromDistMatrixFile (ifstream& inf, MComplex<CC,BC>& ccomp, RIPS<CC, double, BC>& rcomp, bool iscorrmat = true)
{
...
// : [ etc ]
bool buildSToplexFromFile (ifstream& inf, MComplex<CC,BC>& ccomp, SToplex<CC,double,BC>& stop)
{
	pair<num,bool> iores = stop.makeFromFile(inf);
	inf.close();
...
bool buildNMFSToplexFromFile (ifstream& inf, MComplex<CC,BC>& ccomp, SToplex<CC,double,BC>& stop, bool tocap = false)
{
	pair<num,bool> iores = stop.makeFromFile(inf,true,false,tocap);
	inf.close();
