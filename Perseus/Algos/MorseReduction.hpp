                                     ...
                                     bool deep = true, bool tracer = false)
{
	...
	while(!Queue.empty())
	{
		...
		{
			debout<<"\n Popping... "<<*queen;
			debout<<" with cb: "<<queen->cb<<" and bd: "<<queen->bd;
			if (MAKEBPS) cin.get();
		}
		...
		else //if (queen->cb.size()==0)
		{
			if (CORETALK || tracer)
			{
				debout<<", No king; Enq bd...";
			}
			//enqueue the boundary of the queue element
			typename CSTRUCT::const_iterator i;
			for (i = queen->getBD().begin(); i != queen->getBD().end(); ++i)
			{
				if (CORETALK || tracer)
				{
					debout<<"\n   enQ attempt: "<<*(i->first)<<" whose Qstat is "<<i->first->isQueued;
				}
				if (!(i->first->isQueued))
				{
					Queue.push_back(i->first);
					i->first->isQueued = true;
				}
			} // for king->cb iter
                                       ...
                                       bool deep = true, bool tracer = false)
{
	...
	while(!Queue.empty())
	{
		...
		{
			debout<<"\n Popping... "<<*king;
			debout<<" with bd: "<<king->bd<<" and cb: "<<king->cb;
			//cin.get();
		}
		...
		else //if (king->bd.size()==0)
		{
			...
			typename CSTRUCT::const_iterator i;
			for (i = king->getCB().begin(); i != king->getCB().end(); ++i)
			{
				if (CORETALK || tracer)
				{
					debout<<"\n   enQ attempt: "<<*(i->first)<<" whose Qstat is "<<i->first->isQueued;
                                            ...
                                            BT except = BANBT, bool tracer = false)
{
	...
	do
	{
		...
		if (CMFTALK || tracer)
		{
			debout<<"\n        coreduce returned: "<<coret.first<<" "<<coret.second;
...
void CoreduceAndUpdate (MComplex<C,BT>*& mycomp, bool savegens = false,
                        double thresh = 0.5, ostream& out = cout)
{
	...
	do
	{
		...
		{
			if (fracrem >= 0.5) // want to move over?
			{
			    // allocate space...
                rcomp = new MComplex<C,BT>;
                // move over to new reduced complex
                mycomp->moveOver(*rcomp,false); // optimize for coreduction
    ...
    do
    {
        ...
        if ( (fracrem > thresh) || (oldfrem > thresh) ) // we want to keep reducing!
		{
			...
			if (fracrem > movewhen) // want to move over?
			{
                ...
                rcomp = new MComplex<C,BT>;
                // move over to new reduced complex
                mycomp->moveOver(*rcomp, redcycle);
                //cout<<" moved over, new size: "<<rcomp->size(); cin.get();
                //remove old complex
                delete mycomp;
                // set old pointer to new complex
...
template <typename C, typename BT>
void AlternateAndUpdate (MComplex<C,BT>*& mycomp,  bool savegens = false, double thresh = 0.5,
                         bool tracer = false, bool superopt = false, ostream& out = cout)
{
    ...
    do
    {
        tht++;
        oldfrem = fracrem;
        mtype = (redcycle) ? "reductions:   " : "coreductions: ";
        oldsz = mycomp->size();
        mycomp->TopAlternator(redcycle,thresh,savegens,tracer);
        newsz = mycomp->size();
        fracrem = (oldsz == 0) ? 1 : float(oldsz-newsz)/float(oldsz);
        ...
        if ( ((fracrem > thresh) || (oldfrem > thresh)) && !savegens) // we want to keep reducing!
		{
			if (fracrem > movewhen) // want to move over?
			{
			    // allocate space...
	...
	do
	{
		...
		if (fracrem > thresh && !savegens) // we want to keep reducing!
		{
			if (fracrem >= 0.5) // want to move over?
			{
                ...
                rcomp = new MComplex<C,BT>;
                // move over to new reduced complex
                mycomp->moveOver(*rcomp, true); // optimize for reduction
                //cout<<" moved over, new size: "<<rcomp->size(); cin.get();
                // remove old complex
                delete mycomp;
                // set old pointer to new complex
