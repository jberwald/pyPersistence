...
void MComplex<C,BT>::moveOver(MComplex<C,BT>& other, bool forred)
{
    ...
    // and their iterators:
    typename multiset<Cell<C,BT>*, unitbdord<C,BT> >::iterator bditer;
    typename multiset<Cell<C,BT>*, unitcbord<C,BT> >::iterator cbiter;
...
pair<num,bool> MComplex<C,BT>::getMaxDim(const BT& btime)
{
    ...
    {
        if (lsize(btime,dimit->first)==0) continue; // ignore dimensions with size 0!
...
void MComplex<C,BT>::memorySaver(BT birth = INITBT)
{
	...
	for (compit = this->clist.begin(); compit != this->clist.end(); ++compit)
	{
		...
		for (frit = curf->begin(); frit != curf->end(); ++frit)
		{
			...
			// vector candidates
			if (2*lsize(curbirth,curdim) <= (num)curl->size() )
			{
