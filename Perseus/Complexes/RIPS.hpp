...
{
	Point<PS>* mypt = new Point<PS>; // create point
	//cout<<"\n adding "<<mypt;
...
template <typename C, typename PS, typename BT>
void RIPS<C,PS,BT>::ComputePersistence(num ptdim, map<num, vector<pair<BT,BT> > >& pints)
{

	incRIPS(ptdim); // create rips complex and feed to stoplex...
...
// using ptdim.
template <typename C, typename PS, typename BT>
void RIPS<C,PS,BT>::ComputePersistence(num ptdim, string fname = "output")
{
	//cout<<*this;
...
// of the file
template <typename C, typename PS, typename BT>
pair<num,bool> RIPS<C,PS,BT>::makeFromFile_GrowBalls(ifstream& infile, bool comred = false, bool capped=false, bool witness=false)
{
	...
	double radius; num dimcap = ptdim;
	...
	//cout<<"\n eps: "<<epsilon<<" com rad "<<radius<<" #steps "<<numsteps<<" size "<<stepsize;
	//cin.get();
	...
	for (num i = 0; i < numpts; i++)
	{
	    ...
	    for (num j = 0; j < jend; j++)
	    {
            ...
            if (curdist < epsilon)
            {
                ...
                //cout<<" dist: "<<curdist<<"at i="<<i<<" j="<<j<<" and eps: "<<epsilon;
                //cin.get();
            ...
            else // if dist is larger than epsilon,
            {
                ...
                cursteps = ceil((curdist - epsilon)/stepsize);
...
template <typename C, typename PS, typename BT>
void RIPS<C,PS,BT>::incRIPS(const num topdim)
{
	INSRIP dimsimps; // simplices sorted by dimension (> 1)
...
//		{
//			cout<<"\n Point #"<<cur;
//			typename INSRIP::const_iterator sct;
//			for (sct = dimsimps.begin(); sct != dimsimps.end(); ++sct)
//			{
//				cout << "\n ------- dim "<<sct->first<<" has size "<<sct->second->size();
//			}
//			cin.get();
//		}
		...
		AddRIPSCofaces(zerosimp, nblist, topdim, dimsimps, cur); // now add all cofaces
	}
...
bool RIPS<C,PS,BT>::AddRIPSCofaces(Simplex<C,PS,BT>* tau, const vector<num>& nblist,
		                           num topdim, INSRIP& dimsimps, num callerind)
{
	if (tau == NULL) return false;
	if (RIPSTALK)
	{
	...
	for (niter = nblist.begin(); niter != nblist.end(); ++niter)
	{
		...
		if ((siglist.insert(v)).second) // augment vertices of tau with this
		{
			...
			// and recursively call addcofaces!
			AddRIPSCofaces(sigma, intersects, topdim, dimsimps, callerind);
		}
		else return false;
	}
	return true;
