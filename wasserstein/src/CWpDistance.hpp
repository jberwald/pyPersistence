...
{
	double d = ( fabs( gen1.birth - gen2.birth ) + fabs( gen1.death - gen2.death ) );
	if(p == 1 ) return d;
	else if ( p == 2 ) return d * d;
	else return pow(d, p);
}
...
{
	/* This distance is a little more the the distance for
	 * diagonal but is integer and makes the computation faster*/
	if( p == 1 ) return (gen.death - gen.birth);
	else if ( p == 2 ) return ( (gen.death - gen.birth) * (gen.death - gen.birth) );
	else return (int)( pow( (gen.death - gen.birth), p ) );
}
