...
void CBottleneckDistance::LoadGeneratorsFromFile(const char* fileName, std::vector<Generator> &generators, double maxLevel ){
	...
	if(!infile){
		std::cout<< " Unable to open input file " << fileName << ". I'll work with an empty persistence diagram! \n";
		return;
	}
	...
	while(!infile.eof())
	{
		...
		if( NeglectShortGenerators && ( gen.death - gen.birth <= NeglectSize ) ) continue;
...
//		std::cout << i << " -------- "<< Edges[i].vertex_1 << " " << Edges[i].vertex_2 << " " << Edges[i].weight << "\n";
...
double CBottleneckDistance::Distance(const char* diagram_1, const char* diagram_2, double maxLevel ){
	...
	/* First non added edge is an iterator pointing to the first edge in Edges which was added to the Connections */
	...
	while( matching < Max_Size){
		...
		if( matching == Max_Size ) return current_weight;
